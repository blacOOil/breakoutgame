...
class Paddle:
    def __init__(self, skin=1):
        ...
        # Track the previous state of the spacebar to avoid rapid changes
    ...
    def update(self, dt):
        ...
        if key[pygame.K_x]:
            if not self.spacebar_pressed:
                ...
                # Change the skin, loop around if it's the last skin
                self.skin += 1
                if self.skin > len(paddle_image_list):
                    self.skin = 1
                self.SetImage(self.skin)
        else:
            self.spacebar_pressed = False
